...
function NameplateUtil.print(msg)
...
function NameplateUtil.decimal_round(n, dp)      -- ROUND TO 1 DECIMAL PLACE
    local shift = 10^(dp or 0)
    return floor(n*shift + .5)/shift
end

function NameplateUtil.newclass(name)
  if not name then
    print(debug.traceback('new class has no name.'))
  end
...
function NameplateUtil.extends(base, name)
  if not name then
    print(debug.traceback('extended class has no name.'))
  end
...
function NameplateUtil.getTimeDifference(tEnd)
  local t = tEnd - GetTime()
  if t > 5 then return NameplateUtil.decimal_round(t, 0) else return NameplateUtil.decimal_round(t, 1) end
end
...
function table.compare(t, other)
  if table.size(t) ~= table.size(other) then return false end
  for k,v in pairs(t) do
